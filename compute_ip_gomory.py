...
def main():
    ...
    def create_gomorycut(solution):
        ...
        shutil.move((arguments["gams_input_file_prosumer"] + ".log"), save_result_dir)
    ...
    def rewrite_parameter_calculation(number):
        with open("calculate_parameters.dat","a") as pfile:
            for var in colvar:
                pfile.write("loop(Gamma, gomory_" + var + "(Gamma) = gomory_" + var + "(Gamma) + gomory" + str(cutcounter) + "_" + var + "(Gamma) * l_gomory_cut_" + str(cutcounter) + ".l;);\n" )
        return
